(function () {
  ...
  const els = {
    listView: $("listView"), editorView: $("editorView"), backgroundInput: $("backgroundInput"), clearBackgroundBtn: $("clearBackgroundBtn"), templateStage: $("templateStage"), templateImage: $("templateImage"), templateEmpty: $("templateEmpty"), templateAvatarZone: $("templateAvatarZone"), templateNameZone: $("templateNameZone"), nameEnabled: $("nameEnabled"), nameControls: $("nameControls"), nameColor: $("nameColor"), avatarRadius: $("avatarRadius"), avatarBorder: $("avatarBorder"), avatarAspect: $("avatarAspect"), sampleAvatarInput: $("sampleAvatarInput"), fontFamily: $("fontFamily"), fontFamilyZh: $("fontFamilyZh"), fontFamilyEn: $("fontFamilyEn"), fontFamilyJa: $("fontFamilyJa"), fontSizeMode: $("fontSizeMode"), fontSize: $("fontSize"), nameAlign: $("nameAlign"), nameVertical: $("nameVertical"), letterSpacing: $("letterSpacing"), fontInputZh: $("fontInputZh"), fontInputEn: $("fontInputEn"), fontInputJa: $("fontInputJa"), fontStatus: $("fontStatus"), xlsxInput: $("xlsxInput"), folderInput: $("folderInput"), folderField: $("folderField"), embeddedField: $("embeddedField"), modeDescription: $("modeDescription"), importBtn: $("importBtn"), xlsxStatus: $("xlsxStatus"), folderStatus: $("folderStatus"), importStatus: $("importStatus"), itemsGrid: $("itemsGrid"), itemsCount: $("itemsCount"), filenamePrefix: $("filenamePrefix"), exportBtn: $("exportBtn"), exportStatus: $("exportStatus"), exportProgress: $("exportProgress"), backToListBtn: $("backToListBtn"), editorTitle: $("editorTitle"), editorStatus: $("editorStatus"), editorStage: $("editorStage"), editorImage: $("editorImage"), editorAvatarZone: $("editorAvatarZone"), editorNameZone: $("editorNameZone"), editorNameInput: $("editorNameInput"), editorRemarkInput: $("editorRemarkInput"), editorAvatarInput: $("editorAvatarInput"), editorAvatarStatus: $("editorAvatarStatus"), removeEditorAvatarBtn: $("removeEditorAvatarBtn"), cropEditorBtn: $("cropEditorBtn"), editorFontFamily: $("editorFontFamily"), editorFontFamilyZh: $("editorFontFamilyZh"), editorFontFamilyEn: $("editorFontFamilyEn"), editorFontFamilyJa: $("editorFontFamilyJa"), editorFontSize: $("editorFontSize"), editorNameColor: $("editorNameColor"), editorNameAlign: $("editorNameAlign"), editorNameVertical: $("editorNameVertical"), resetItemBtn: $("resetItemBtn"), setGlobalBtn: $("setGlobalBtn"), downloadCurrentBtn: $("downloadCurrentBtn"), cropDialog: $("cropDialog"), cropImage: $("cropImage"), cancelCropBtn: $("cancelCropBtn"), saveCropBtn: $("saveCropBtn")
  };
  ...
  function defaultTemplate() {
    return {
      ...
      avatarBox: { x: 300, y: 260, width: 400, height: 400, cropAspectRatio: 1, borderRadius: 24, borderWidth: 0, borderColor: "#ffffff" },
      nameBlock: { enabled: true, x: 180, y: 760, width: 640, height: 120, fontId: "NotoSansCJKsc", cssFontFamily: "Noto Sans CJK SC", fontFamilies: { zh: "Noto Sans SC", en: "Noto Sans", ja: "Noto Sans JP" }, fontSizeMode: "auto", fontSize: 64, fontSizeRange: { min: 24, max: 72 }, color: "#18221d", align: "center", verticalAlign: "middle", letterSpacing: 0 }
    };
  }
  ...
  function readTemplateInputs() {
    ...
    a.cropAspectRatio = ratio; a.borderRadius = Math.max(0, number(els.avatarRadius.value, a.borderRadius)); a.borderWidth = Math.max(0, number(els.avatarBorder.value, a.borderWidth)); constrainBox(a, ratio);
    n.enabled = els.nameEnabled.checked; n.fontFamilies = { zh: els.fontFamilyZh.value, en: els.fontFamilyEn.value, ja: els.fontFamilyJa.value }; n.cssFontFamily = n.fontFamilies.zh; n.fontId = n.fontFamilies.zh; n.fontSizeMode = els.fontSizeMode.value; n.fontSize = Math.max(8, number(els.fontSize.value, n.fontSize)); n.color = els.nameColor.value; n.align = els.nameAlign.value; n.verticalAlign = els.nameVertical.value; n.letterSpacing = Math.max(0, number(els.letterSpacing.value, 0)); constrainBox(n, 0);
    updateTemplateInputs();
  }

  function setBackground(opened) {
    if (state.template.backgroundImage) state.template.backgroundImage.release();
  ...
  function drawAvatar(ctx, image, box) {
    if (!image) return;
    ...
    if (sourceRatio > targetRatio) { sw = source.height * targetRatio; sx = (source.width - sw) / 2; } else { sh = source.width / targetRatio; sy = (source.height - sh) / 2; }
    ctx.save(); drawRoundRect(ctx, box.x, box.y, box.width, box.height, box.borderRadius); ctx.clip(); ctx.drawImage(image, sx, sy, sw, sh, box.x, box.y, box.width, box.height); ctx.restore();
  ...
  function drawName(ctx, text, block) {
    ...
    let size = block.fontSize || 48; const min = block.fontSizeRange ? block.fontSizeRange.min : 18; const max = block.fontSizeRange ? block.fontSizeRange.max : size;
    if (block.fontSizeMode === "auto") { let lo = min; let hi = max; while (lo <= hi) { const mid = Math.floor((lo + hi) / 2); if (measureName(ctx, text, block, mid) <= block.width) { size = mid; lo = mid + 1; } else hi = mid - 1; } }
  ...
  async function renderItem(item, scale) {
    ...
    let avatarHandle = null; if (item.avatarSource) avatarHandle = await imageElementFromBlob(item.avatarSource); drawAvatar(ctx, avatarHandle && avatarHandle.image, mergedBox(item)); drawName(ctx, item.name, mergedName(item)); if (avatarHandle) avatarHandle.release(); ctx.restore(); return canvas;
  }
  async function thumbnail(item) { const canvas = await renderItem(item, Math.min(1, 200 / state.template.backgroundWidth)); return canvas.toDataURL("image/jpeg", .82); }
  async function updateTemplatePreview() {
    ...
    const canvas = await renderItem(fakeItem, Math.min(1, 760 / state.template.backgroundWidth));
    els.templateImage.src = canvas.toDataURL("image/png");
  }

  function setStatus(target, text) { target.textContent = text || ""; }
  function renderItems() {
    els.itemsCount.textContent = state.items.length ? state.items.length + " 条记录" : "尚未导入数据"; els.exportBtn.disabled = !state.items.length;
  ...
  async function parseXlsx(file, includeImages) {
    ...
    const records = []; for (const [rowNumber, cells] of rows) { if (rowNumber === 1 && /姓名|name/i.test(cells[0] || "")) continue; const name = normalizeName(cells[0]); const remark = String(cells[2] || "").trim(); if (!name && !remark) continue; let avatar = null; if (includeImages && rowToImage.has(rowNumber)) { const image = rowToImage.get(rowNumber); if (/\.(?:emf|wmf)$/i.test(image.path)) throw new Error("XLSX_VECTOR_IMAGE"); avatar = await readAvatar(new Blob([await image.file.async("arraybuffer")], { type: imageMime(image.path) })); } records.push({ rowNumber, name, remark, avatar }); }
  ...
  async function importRecords() {
    ...
    if (!els.xlsxInput.files[0]) { setStatus(els.importStatus, "请先选择 XLSX 表格。"); return; }
    els.importBtn.disabled = true; setStatus(els.importStatus, "正在读取表格...");
    try {
      const records = await parseXlsx(els.xlsxInput.files[0], state.mode === "embedded"); const matched = new Set(); let unused = 0; const folderMap = new Map();
      if (state.mode === "folder") { state.folderFiles.forEach(file => { if (M.isImageFile(file)) folderMap.set(extless(file.name), file); }); }
      state.items = [];
      for (const record of records) { let avatar = record.avatar; let ok = Boolean(avatar); if (state.mode === "folder") { const file = folderMap.get(normalizeName(record.name)); if (file) { avatar = await readAvatar(file); matched.add(file); ok = true; } else ok = false; } const item = { id: id(), name: record.name, remark: record.remark, avatarSource: avatar, avatarOverride: null, nameOverride: null, matched: ok, status: ok ? "ready" : "missing-avatar" }; item.thumbnail = await thumbnail(item); state.items.push(item); }
      if (state.mode === "folder") unused = state.folderFiles.filter(file => M.isImageFile(file) && !matched.has(file)).length;
      renderItems(); els.importBtn.disabled = false; setStatus(els.importStatus, "已导入 " + state.items.length + " 条记录" + (unused ? "，有 " + unused + " 个头像文件未被使用。" : "。") + (state.items.filter(item => item.status === "missing-avatar").length ? " 缺少头像的记录已标记。" : ""));
    } catch (error) {
  ...
  function applyEditorInputs() { const item = currentItem(); if (!item) return; const n = mergedName(item); item.name = els.editorNameInput.value; item.remark = els.editorRemarkInput.value; item.nameOverride = Object.assign({}, item.nameOverride || {}, { cssFontFamily: els.editorFontFamilyZh.value, fontId: els.editorFontFamilyZh.value, fontFamilies: { zh: els.editorFontFamilyZh.value, en: els.editorFontFamilyEn.value, ja: els.editorFontFamilyJa.value }, fontSize: Math.max(8, number(els.editorFontSize.value, n.fontSize)), color: els.editorNameColor.value, align: els.editorNameAlign.value, verticalAlign: els.editorNameVertical.value }); updateEditorInputs(); thumbnail(item).then(data => { item.thumbnail = data; renderItems(); }); updateEditorPreview(); }
  ...
  function setGlobal() { const item = currentItem(); if (!item) return; state.template.avatarBox = Object.assign({}, state.template.avatarBox, item.avatarOverride || {}); state.template.nameBlock = Object.assign({}, state.template.nameBlock, item.nameOverride || {}); updateTemplateInputs(); refreshCurrentAndItems(); setStatus(els.editorStatus, "当前条目的覆盖设置已作为新的全局默认。其他条目会按新的默认重新生成预览。"); }

  async function cropFile(file, target) { const opened = await M.open(file); els.cropImage.src = opened.src; els.cropDialog.hidden = false; state.cropTarget = { target, opened }; try { await M.ensureCropper(); state.cropper = new window.Cropper(els.cropImage, { viewMode: 1, autoCropArea: .85, aspectRatio: target === "template" ? state.template.avatarBox.cropAspectRatio || NaN : editorBox(currentItem()).cropAspectRatio || NaN }); } catch (error) { state.cropper = null; setStatus(els.editorStatus, "裁剪组件加载失败，将使用居中裁剪。"); } }
  async function saveCrop() { if (!state.cropTarget) return; const { target, opened } = state.cropTarget; let blob; if (state.cropper) blob = await new Promise(resolve => state.cropper.getCroppedCanvas({ maxWidth: 1200, maxHeight: 1200 }).toBlob(resolve, "image/jpeg", .9)); else blob = await readAvatar(opened.blob); opened.release(); if (target === "template") { state.template.sampleAvatar = await readAvatar(blob); updateTemplatePreview(); } else { const item = currentItem(); item.avatarSource = await readAvatar(blob); item.matched = true; item.status = "ready"; item.thumbnail = await thumbnail(item); renderItems(); updateEditorInputs(); await updateEditorPreview(); } if (state.cropper) state.cropper.destroy(); state.cropper = null; state.cropTarget = null; els.cropDialog.hidden = true; }

  async function exportOne(item) { const canvas = await renderItem(item, 1); return U.canvasToBlob(canvas, "image/png"); }
  async function exportAll() { if (!state.items.length) return; els.exportBtn.disabled = true; els.exportProgress.style.width = "0%"; const files = []; const used = new Map(); try { for (let i = 0; i < state.items.length; i += 1) { setStatus(els.exportStatus, "正在生成 " + (i + 1) + "/" + state.items.length + "..."); const item = state.items[i]; const name = U.sanitizeFilename((els.filenamePrefix.value || "") + (item.name || "未命名") + (item.remark ? "_" + item.remark : ""), "未命名"); files.push({ name: U.uniqueFilename(name, used), blob: await exportOne(item) }); els.exportProgress.style.width = Math.round((i + 1) / state.items.length * 100) + "%"; await new Promise(resolve => setTimeout(resolve, 0)); } await U.exportZip(files, "证件批量导出.zip"); setStatus(els.exportStatus, "已完成 " + state.items.length + " 张证件并下载 ZIP。"); } catch (error) { setStatus(els.exportStatus, "导出失败：" + (error.message === "NO_BACKGROUND" ? "请先设置背景图。" : errorText(error))); } els.exportBtn.disabled = false; }
  ...
  els.backgroundInput.addEventListener("change", async () => { const file = els.backgroundInput.files[0]; if (!file) return; try { setBackground(await M.open(file)); } catch (error) { setStatus(els.importStatus, errorText(error)); } });
  els.clearBackgroundBtn.addEventListener("click", () => { if (state.template.backgroundImage) state.template.backgroundImage.release(); state.template.backgroundImage = null; state.template.sampleAvatar = null; els.templateStage.classList.remove("has-background"); els.editorStage.classList.remove("has-background"); els.templateImage.hidden = true; els.templateEmpty.hidden = false; updateTemplateZones(); });
  els.sampleAvatarInput.addEventListener("change", () => { const file = els.sampleAvatarInput.files[0]; if (file) cropFile(file, "template"); });
  [els.avatarRadius, els.avatarBorder, els.avatarAspect, els.nameEnabled, els.fontFamilyZh, els.fontFamilyEn, els.fontFamilyJa, els.fontSizeMode, els.fontSize, els.nameColor, els.nameAlign, els.nameVertical, els.letterSpacing].forEach(el => el.addEventListener("input", readTemplateInputs));
  els.avatarAspect.addEventListener("change", readTemplateInputs);
  async function registerFont(file, language, select, editorSelect) { if (!file) return; const family = "OnlyCustom_" + language + "_" + id(); try { const face = new FontFace(family, "url(" + URL.createObjectURL(file) + ")"); await face.load(); document.fonts.add(face); state.customFonts.set(family, { id: family, language, fileName: file.name, fileBlob: file }); const option = document.createElement("option"); option.value = family; option.textContent = file.name + "（自定义）"; select.appendChild(option); editorSelect.appendChild(option.cloneNode(true)); select.value = family; editorSelect.value = family; readTemplateInputs(); setStatus(els.fontStatus, "已注册" + ({ zh: "中文", en: "英文", ja: "日文" }[language]) + "字体：" + file.name + "。"); } catch (error) { setStatus(els.fontStatus, "字体加载失败，请确认文件为 ttf、otf、woff 或 woff2。"); } }
  [[els.fontInputZh, "zh", els.fontFamilyZh, els.editorFontFamilyZh], [els.fontInputEn, "en", els.fontFamilyEn, els.editorFontFamilyEn], [els.fontInputJa, "ja", els.fontFamilyJa, els.editorFontFamilyJa]].forEach(([input, language, select, editorSelect]) => input.addEventListener("change", () => registerFont(input.files[0], language, select, editorSelect)));
  document.querySelectorAll("[data-mode]").forEach(button => button.addEventListener("click", () => { state.mode = button.dataset.mode; document.querySelectorAll("[data-mode]").forEach(other => { other.classList.toggle("active", other === button); other.setAttribute("aria-selected", other === button ? "true" : "false"); }); const folder = state.mode === "folder"; els.folderField.hidden = !folder; els.embeddedField.hidden = folder; els.modeDescription.textContent = folder ? "上传只含姓名第一列、备注第三列的 xlsx，再选择头像文件夹，按文件名去扩展名后与姓名精确匹配。" : "上传第二列带内嵌头像的 xlsx。程序会尽力读取标准图片锚点；解析失败时请改用文件夹匹配。"; }));
  ...
  els.backToListBtn.addEventListener("click", () => { applyEditorInputs(); closeEditor(); }); els.editorNameInput.addEventListener("input", applyEditorInputs); els.editorRemarkInput.addEventListener("input", applyEditorInputs); [els.editorFontFamilyZh, els.editorFontFamilyEn, els.editorFontFamilyJa, els.editorFontSize, els.editorNameColor, els.editorNameAlign, els.editorNameVertical].forEach(el => el.addEventListener("input", applyEditorInputs));
