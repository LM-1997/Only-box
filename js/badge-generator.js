(function () {
  ...
  function defaultTemplate() {
    return {
      ...
      nameBlock: { enabled: true, x: 180, y: 760, width: 640, height: 120, fontId: "NotoSansCJKsc", cssFontFamily: "Noto Sans CJK SC", fontFamilies: { zh: "Noto Sans SC", en: "Noto Sans", ja: "Noto Sans JP" }, weight: 700, fontSize: "", fontSizeRange: { min: 24, max: 72 }, color: "#18221d", align: "center", verticalAlign: "middle", letterSpacing: 0 }
    };
  }
  ...
  function readTemplateInputs() {
    ...
    n.enabled = els.nameEnabled.checked; n.fontFamilies = { zh: els.fontFamilyZh.value, en: els.fontFamilyEn.value, ja: els.fontFamilyJa.value }; n.cssFontFamily = n.fontFamilies.zh; n.fontId = n.fontFamilies.zh; n.weight = nameWeight({ weight: number(els.nameWeight.value, 700) }); const fontSizeValue = String(els.fontSize.value || "").trim(); n.fontSize = fontSizeValue && Number(fontSizeValue) > 0 ? Math.max(8, Number(fontSizeValue)) : ""; n.color = els.nameColor.value; n.align = els.nameAlign.value; n.verticalAlign = els.nameVertical.value; n.letterSpacing = Math.max(0, number(els.letterSpacing.value, 0)); constrainBox(n, 0);
  ...
  /* 姓名字重：block.weight 由字重控件选择（旧配置缺省回落 700，与历史行为一致） */
  function nameWeight(block) { const w = Number(block && block.weight); return w >= 100 && w <= 900 ? w : 700; }
  function measureName(ctx, text, block, size) {
    let width = 0;
    for (const ch of String(text)) { ctx.font = nameWeight(block) + " " + size + "px " + fontFamilyForChar(ch, block); width += ctx.measureText(ch).width; }
  ...
  function drawNameCharacters(ctx, text, block, x, baseline, size) {
    let cursor = x;
    for (const ch of String(text)) { ctx.font = nameWeight(block) + " " + size + "px " + fontFamilyForChar(ch, block); ctx.fillText(ch, cursor, baseline); cursor += ctx.measureText(ch).width + (block.letterSpacing || 0); }
  }
  ...
  async function renderItem(item, scale, opts) {
    ...
    /* canvas 不触发字体下载：绘制前按姓名文本 × 语言字体 × 当前字重确保 webfont 就绪（预览与导出一致） */
    const block = mergedName(item);
    try { if (window.BadgeFontPicker) await window.BadgeFontPicker.ensureFontsForBlock(item.name || "示例姓名", block); } catch (e) { /* 字体未就绪时按回退字体渲染 */ }
    let avatarHandle = null; if (item.avatarSource) avatarHandle = await imageElementFromBlob(item.avatarSource); drawAvatar(ctx, avatarHandle && avatarHandle.image, mergedBox(item), avatarAlpha); drawName(ctx, item.name, block); if (avatarHandle) avatarHandle.release(); ctx.restore(); return canvas;
  }
  ...
  function applyEditorInputs() { const item = currentItem(); if (!item) return; const n = mergedName(item); const fontSizeValue = String(els.editorFontSize.value || "").trim(); item.name = els.editorNameInput.value; item.remark = els.editorRemarkInput.value; item.nameOverride = Object.assign({}, item.nameOverride || {}, { cssFontFamily: els.editorFontFamilyZh.value, fontId: els.editorFontFamilyZh.value, fontFamilies: { zh: els.editorFontFamilyZh.value, en: els.editorFontFamilyEn.value, ja: els.editorFontFamilyJa.value }, weight: nameWeight({ weight: number(els.editorNameWeight.value, n.weight || 700) }), fontSize: fontSizeValue && Number(fontSizeValue) > 0 ? Math.max(8, Number(fontSizeValue)) : "", color: els.editorNameColor.value, align: els.editorNameAlign.value, verticalAlign: els.editorNameVertical.value }); updateEditorInputs(); thumbnail(item).then(data => { item.thumbnail = data; renderItems(); }); updateEditorPreview(); }
  ...
  [els.avatarRadius, els.avatarBorder, els.avatarBorderColor, els.avatarBaseEnabled, els.avatarBaseColor, els.avatarOutlineWidth, els.avatarOutlineGap, els.avatarOutlineColor, els.avatarAspect, els.nameEnabled, els.fontFamilyZh, els.fontFamilyEn, els.fontFamilyJa, els.nameWeight, els.fontSize, els.nameColor, els.nameAlign, els.nameVertical, els.letterSpacing].forEach(el => el.addEventListener("input", readTemplateInputs));
  els.avatarAspect.addEventListener("change", readTemplateInputs);
    ...
    async function registerFont(file, language) {
      if (!file) return;
      /* 解析字体文件真实元数据：family（name 表）、字重档（OS/2 usWeightClass）、
         可变字体 wght 轴范围（fvar 表）——静态字体按真实字重注册单一档位，
         可变字体注册全轴范围并在字重控件中显示滑杆。 */
      let meta = { family: null, format: null, weightClass: null, isVariable: false };
      try { meta = window.OnlyBoxFontMeta.parse(await file.arrayBuffer()); } catch (e) { /* 解析失败走兜底命名 */ }
      const family = meta.family || "OnlyCustom_" + language + "_" + id();
      const isVariable = Boolean(meta.isVariable);
      const weightClass = Math.min(900, Math.max(100, Number(meta.weightClass) || 400));
      try {
        const descriptors = isVariable && meta.wghtMin != null && meta.wghtMax != null
          ? { weight: meta.wghtMin + " " + meta.wghtMax }
          : { weight: String(weightClass) };
        const face = new FontFace(family, "url(" + URL.createObjectURL(file) + ")", descriptors);
        await face.load();
        document.fonts.add(face);
        state.customFonts.set(family, { id: family, language, fileName: file.name, fileBlob: file, meta: Object.assign({}, meta, { isVariable, weightClass }) });
        const option = document.createElement("option");
        option.value = family;
        option.textContent = file.name + "（自定义）";
        // 同步到全部六个语言下拉（模板 中/英/日 + 编辑器 中/英/日），保证单字体可被任意语言类型复用
        ...
        if (window.BadgeFontPicker) window.BadgeFontPicker.refreshCustomFonts();
        applyCustomFontSelection();
        const weightNote = isVariable && meta.wghtMin != null
          ? "可变字体，字重轴 " + meta.wghtMin + "-" + meta.wghtMax + "（可在字重控件拖动）"
          : "静态字体，字重 " + weightClass;
        setStatus(els.fontStatus, "已注册" + CUSTOM_FONT_LABELS[language] + "字体：" + file.name + "（" + weightNote + "）。");
      } catch (error) {
  ...
  els.itemsGrid.addEventListener("change", async event => { if (!event.target.classList.contains("quick-avatar")) return; const card = event.target.closest(".item-card"); const item = state.items.find(entry => entry.id === card.dataset.id); const file = event.target.files[0]; if (!item || !file) return; try { item.avatarSource = await readAvatar(file); item.matched = true; item.status = "ready"; item.thumbnail = await thumbnail(item); renderItems(); } catch (error) { setStatus(els.importStatus, errorText(error)); } });
    els.backToListBtn.addEventListener("click", () => { applyEditorInputs(); closeEditor(); }); els.editorNameInput.addEventListener("input", applyEditorInputs); els.editorRemarkInput.addEventListener("input", applyEditorInputs); [els.editorFontFamilyZh, els.editorFontFamilyEn, els.editorFontFamilyJa, els.editorNameWeight, els.editorFontSize, els.editorNameColor, els.editorNameAlign, els.editorNameVertical].forEach(el => el.addEventListener("input", applyEditorInputs)); [els.editorAvatarAspect, els.editorAvatarRadius, els.editorAvatarBorder, els.editorAvatarBorderColor, els.editorAvatarBaseEnabled, els.editorAvatarBaseColor, els.editorAvatarOutlineWidth, els.editorAvatarOutlineGap, els.editorAvatarOutlineColor].forEach(el => el.addEventListener("input", applyEditorAvatarInputs)); els.editorAvatarAspect.addEventListener("change", applyEditorAvatarInputs);
  ...
  window.addEventListener("resize", () => { updateTemplateZones(); updateEditorZones(); });
  /* 统一字体选择器：本文件先于 badge-font-picker.js 加载，此处只暴露配置；
     选择器模块加载完成后自初始化（内置清单 + 自定义上传字体同面板）。 */
  window.__badgeFontPickerConfig = {
    getCustomFonts: () => state.customFonts,
    rerenderAll: () => window.__badgeRerenderAll()
  };
  if (window.BadgeFontPicker && !window.BadgeFontPicker.ready) window.BadgeFontPicker.init(window.__badgeFontPickerConfig);
  updateTemplateInputs();
})();
